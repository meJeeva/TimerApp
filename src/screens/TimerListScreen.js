...
const TimerListScreen = () => {
    ...
    const updateTimers = () => {
        setTimerStates((prev) => {
            ...
            Object.keys(updatedTimers).forEach(async (id) => {
                if (updatedTimers[id].status === "Running" && updatedTimers[id].remainingTime > 0) {
                    updatedTimers[id].remainingTime -= 1;
                }

                if (updatedTimers[id].remainingTime === 0 && updatedTimers[id].status !== "Completed") {
    ...
    const startAllTimers = (category) => {
        setTimerStates((prev) => {
            const updated = { ...prev };
            timers.filter((t) => t.category === category).forEach((timer) => {
                ...
                renderItem={({ item: category }) => (
                    <View>
                        ...
                        {expandedCategories[category] && (
                            <View>
                                {/* Start, Pause, Reset All Buttons */}
                                <View style={styles.bulkActions}>
                                    <TouchableOpacity style={styles.bulkButton} onPress={() => startAllTimers(category)}>
                                        <Text style={styles.bulkButtonText}>Start All</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={styles.bulkButton} onPress={() => pauseAllTimers(category)}>
                                        <Text style={styles.bulkButtonText}>Pause All</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={styles.bulkButton} onPress={() => resetAllTimers(category)}>
                                ...
                                {groupedTimers[category].map((timer) => (
                                    <View key={timer.id} style={styles.timerItem}>
                                        ...
                                        <Progress.Bar progress={timerStates[timer.id]?.remainingTime / timer.duration} width={200} />
                                        <Text>{Math.round((timerStates[timer.id]?.remainingTime / timer.duration) * 100)}% remaining</Text>
                                        <TouchableOpacity style={styles.toggleButton} onPress={() => toggleTimer(timer.id)}>
                                            <Text style={styles.buttonText}>{timerStates[timer.id]?.status === "Running" ? "Pause" : "Start"}</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={styles.resetButton} onPress={() => resetTimer(timer.id)}>
...
const styles = StyleSheet.create({
    ...
    categoryHeader: { flexDirection: "row", justifyContent: "space-between", padding: 12, backgroundColor: "#ddd", marginVertical: 5, borderRadius: 8 },
    bulkActions: { flexDirection: "row", justifyContent: "space-around", marginVertical: 10 },
    bulkButton: { backgroundColor: "#007bff", padding: 8, borderRadius: 5 },
    bulkButtonText: { color: "#fff", fontWeight: "bold" },
    timerItem: { backgroundColor: "#fff", padding: 15, borderRadius: 10, marginVertical: 8, elevation: 2 },
